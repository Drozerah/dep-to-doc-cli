 ...
 * - \[Drozerah](https://github.com/drozerah) or 'Link not available' content
 ...
 * @returns {string} The mardown formated link or alternate 'Link not available' text content.
 */
module.exports.createMarkdownLink = function (link_url, link_text) {
  ...
  const markdown_link = typeof link_url === 'undefined' ? 'Link not available' : `[${link_text}](${link_url})`
  debug(' ↪ markdown_link:\n', markdown_link)
 ...
 *
 * - retrieve each package properties from node_modules package.json
 * - update input data Object with values from package.json
  ...
  return new Promise((resolve, reject) => {
    ...
    const parseModules = key => {
      ...
      const map = data[key].map((package_name) => {
        ...
        // const markdown_link = typeof homepage === 'undefined' ? 'Link not available' : `[${package_name}](${homepage})`
